...
- [x] 16-06-PLAN.md - abandoned-run sweep + hourly pg_cron wiring + [BLOCKING] apply (W3) — **built as direct pg_cron SQL, NOT the planned pg_net→demo-sweep edge fn** (orchestrator-endorsed deviation: 16-SPEC.md:113 justifies an edge fn solely by "Storage-object removal", but 16-SPEC.md:198 says that shared canonical file is "retained — not deleted per run" and `demo-run-start:359` confirms nothing is uploaded per run, so the edge fn bought nothing while pg_net needed an unverifiable Vault bearer whose failure mode is a silent hourly 401). LIVE on fuuvdcvbliijffogjnwg: `public.sweep_abandoned_demo_runs(p_max_batch, p_dry_run)` + active hourly job `demo-run-sweep`. **First genuine live verification of Phase 16:** a REAL non-dry invocation ran against production holding **66 proposals + 38 proposal_documents and deleted nothing** — guards exercised against real client data, not an empty table. Live dry run caught a `min(uuid)` 42883 error that would have made the hourly job fail silently forever (fixed `339153d`). Teardown duplication with `_shared/demoRunCleanup.ts` is fenced by a mutation-verified parity test. Still unproven: the 24h age path, per-row exception isolation and the skipped_guard branch (no abandoned run has ever existed)
- [x] 16-07-PLAN.md - capture UI on ProposalDetail + remove vestigial Reset Demo/labels (W3) — **the deadlock-breaking plan: the first UI able to write a `demo_fixtures` row**, which is the single thing blocking the deferred end-to-end verifications in 16-03/16-04/16-05/16-06. `src/components/SaveAsDemoFixtureButton.tsx` (extracted from ProposalDetail, which cannot be mounted in Vitest) invokes `demo-capture-fixture` with `{ source_proposal_id }` and reports the allocated version; gated on `super_admin` **AND** the caller's own org resolving as demo at runtime (`feature_flags.is_demo` / `jamo-demo` slug, no hardcoded UUID) because two super_admins exist and a role-only gate 403s for the internal one. Surfaces the edge fn's own error text by reading `error.context.json().error` — supabase-js otherwise discards every precise 400/403 (e.g. `ungenerated section(s): <names>`) behind a generic wrapper message. D-09: the `sessionStorage.clear()+reload` "Reset Demo" button (zero DB deletes) and both `jamo Demo v0.1.0` labels are gone from `src/` entirely; no replacement (the real run-scoped reset is 16-09). Frontend-only — nothing deployed, nothing live touched. 9 new tests; suite 470 passed / 16 skipped / 0 failed. **Still unexercised:** no fixture captured — that needs a human presenter session (demo-org proposal → real ingest → full real generation → click Save as demo fixture)
- [x] 16-08-PLAN.md - presenter run surface: wizard driver + template lock + paced populate + Req 6 guard (W3) — `/demo` (routed behind `ProtectedRoute`→`SuperAdminRoute`→`Layout`; App.tsx was in no Phase 16 plan's file list, but an unrouted surface would have blocked 16-09's E2E verify) renders `DemoRunSurface` driven by `useDemoRun`: invokes `demo-run-start` with the standard template, **adopts the returned proposal_id** (never mints a second draft), reads back server-materialized assumptions, exposes `demoRunId` for 16-09's reset, and paces the reveal by dispatching `SECTION_COMPLETE` per row into the **real** `generationReducer` at 350 ms — no generation call, no simulated streaming. Reuses the real Step2/Step3/Step4; `Step4Generate` gained `demoMode` (D-02) forcing the `is_default` template with every card `aria-disabled`/`pointer-events-none` and **no onClick**. **Caught a live-model-call bug:** the reused `Step2DocumentUpload` fires `extract-assumptions` whenever documents are parsed and `extractionStatus==='idle'`, and a demo run materializes an already-parsed document — so every demo would have spent a model call and overwritten the fixture assumptions. Fixed by seeding `extractionStatus='complete'` at run adoption — **no branch added to Step2** (the fix is what the step is *given*, not what it *does*). Req 6 fenced by `src/__tests__/no-demo-branch-below-population.test.ts`, which sweeps whole edge-fn directories, not just the five named files; mutation-verified with probes in `retrieve-context/index.ts` and `chat-with-jamo/rag.ts` (the latter caught **only** by the sweep), and the mutation run exposed that `\bdemo_run\b` does not match `demo_run_id`. Reqs SPEC-R4/R6. Frontend-only. Suite 492 passed / 16 skipped / 0 failed (+22)
- [x] 16-09-PLAN.md - in-session run-scoped reset control + presenter E2E human-verify (W4) — **WAVE 4 COMPLETE; all 9 plans of Phase 16 are now coded.** `src/components/demo/DemoResetControl.tsx` invokes the deployed `demo-reset` with **the `demo_run_id` this session started** (D-10 — the demo login is shared, so an account-inferred "current run" could delete the other presenter's live demo mid-sentence) and, only after the server confirms, calls `useDemoRun.resetToStart()` to return to "Add demo RFP" entirely in-session (D-11 — the old `sessionStorage.clear()`+reload deleted nothing server-side and cost a full app boot in front of an audience; the no-reload acceptance grep is now clean and its test spy is wired-checked so the assertion cannot pass vacuously). One behavior, no modes, no bulk option (D-12 — bulk cleanup is 16-06's sweep). Mounted only inside `DemoRunSurface` while a run is live, never the global Sidebar. Deviations: `reset` renamed `resetToStart` (plan acceptance greps that name); a two-step confirm added because a one-click irreversible delete on a live presentation surface is a hazard; and my own first-draft comments contained the literal `window.location.reload` token, defeating the D-11 grep fence until reworded (same class as the 15-05 `inviteUserByEmail` precedent). Frontend-only — nothing deployed. Suite **500 passed / 16 skipped / 0 failed** (+8). **Task 3 (live presenter E2E) is an OPEN, DEFERRED human-verify checkpoint** — recorded like 15-09/15-10's. **No live reset has ever been performed and none can be:** `demo_fixtures` is empty (so `demo-run-start` only ever returns `400 no active demo fixture`) and `{demoOrgId}/demo/canonical-demo-rfp.pdf` has not been uploaded. The tests are unit-level against a mocked supabase client; the deployed `demo-reset` is unexercised. **The 10-step end-to-end verification script that clears the deferred verifications for 16-03/16-04/16-05/16-09 in one session lives in `16-09-SUMMARY.md`** — read it before attempting the phase gate

-- Migration: Phase 14.6 — ingest_regulatory_document supersedes raise
-- Hardens the supersession branch of ingest_regulatory_document: when a caller passes a
-- p_supersedes_document_key that does not resolve to an active regulatory_documents row, the
-- function now RAISES instead of silently no-opping. The raise sits inside the
-- `p_supersedes_document_key IS NOT NULL` branch only, so plain non-superseding ingests (all
-- GLOBAL starter docs, where p_supersedes_document_key IS NULL) are entirely unaffected.
-- Applied in Plan 03 (Wave 1).

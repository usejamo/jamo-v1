...
//
// DENO COPY. DUPLICATED (not imported) from src/lib/demoFixtureValidation.ts — the Deno
// edge runtime cannot resolve src/lib/ imports at deploy time (same convention as
// src/lib/slug.ts <-> admin-create-org, per 14.6-PATTERNS).
// KEEP BOTH COPIES IN SYNC. src/lib/__tests__/demoFixtureValidation.test.ts asserts that
// everything below the SHARED LOGIC marker is byte-identical in both files, so a drift in

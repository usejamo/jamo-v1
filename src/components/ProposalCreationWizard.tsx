...
function getInitialState(): WizardState {
  try {
    ...
    if (stored) {
      ...
      if (parsed.stateVersion === 9) return parsed
    }
  } catch {
    // ignore
  }
...
export function ProposalCreationWizard() {
  ...
  useEffect(() => {
    ...
    createProposal({
      ...
      description: JSON.stringify({ services: studyInfo.services, investigationalProduct: studyInfo.investigationalProduct, investigationalProductUndisclosed: studyInfo.investigationalProductUndisclosed }),
    }).then((id) => {
